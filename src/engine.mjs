...
export function trace(source){
 ...
 const root={constants:new Set(),values:Object.create(null),parent:null};let current=root;
 ...
 function put(n,v){if(n.type==='Identifier'){const scope=lookup(n.name);if(scope.constants.has(n.name))throw Error("Assignment to constant variable: "+n.name);scope.values[n.name]=v;return v;}if(n.type==='MemberExpression'){const o=ev(n.object);o[key(n)]=v;return v;}throw Error('Unsupported assignment target');}
 ...
 if(n.callee.type==='MemberExpression'){const obj=ev(n.callee.object),k=key(n.callee),args=n.arguments.map(ev);if(Array.isArray(obj)&&['push','pop','shift','unshift','slice','join','indexOf','includes','reverse'].includes(k)){let result=Array.prototype[k].apply(obj,args);snap(n,'Array.'+k);return result;}throw Error('This method needs a runtime plugin');}
 const f=ev(n.callee);if(!f?.fn)throw Error('Only interpreted functions can be called');if(stack.length>80)throw Error('Call stack limit reached');const args=n.arguments.map(ev),prev=current;current={constants:new Set(),values:Object.create(null),parent:f.scope};f.fn.params.forEach((p,i)=>{if(p.type!=='Identifier')throw Error('Use simple function parameters');current.values[p.name]=args[i];});stack.push(n.callee.name||'anonymous');snap(n,'Call '+stack.at(-1));let result;try{result=f.fn.body.type==='BlockStatement'?exec(f.fn.body):{return:true,value:ev(f.fn.body)};snap(n,'Return '+stack.at(-1));}finally{stack.pop();current=prev;}return result?.value;
 }
 ...
 case 'BlockStatement':{const prev=current;current={constants:new Set(),values:Object.create(null),parent:prev};try{return body(n.body);}finally{current=prev;}}
 case 'FunctionDeclaration':snap(n,'Declare function '+n.id.name);break;
 case 'VariableDeclaration':for(const d of n.declarations){if(d.id.type!=='Identifier')throw Error('Destructuring is not supported');current.values[d.id.name]=ev(d.init);if(n.kind==='const')current.constants.add(d.id.name);snap(n,'Initialize '+d.id.name);}break;
 ...
 case 'ForStatement':{const prev=current;current={constants:new Set(),values:Object.create(null),parent:prev};try{if(n.init)n.init.type==='VariableDeclaration'?exec(n.init):ev(n.init);while(!n.test||ev(n.test)){snap(n,'Loop iteration');const r=exec(n.body);if(r?.return)return r;if(r?.break)break;if(n.update)ev(n.update);}snap(n,'Loop complete');}finally{current=prev;}break;}
